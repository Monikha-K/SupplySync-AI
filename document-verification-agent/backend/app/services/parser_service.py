...
# Common labels to ignore when scanning nearby lines for values
...
def normalize_ocr_text(text: str) -> str:
    ...
    - Standardizes labels (DL No., Date of Issue, Valid Till, Date of Birth).
    - Removes empty lines and trims whitespace.
    - Collapses multiple spaces into single space per line.
    """
    if not text:
        return ""
...
def normalize_dl_number(dl_str: str) -> str:
    """
    Normalizes a Driving Licence number string:
    - Replaces O/o with 0 in numeric sections (e.g. TN6O -> TN60).
    - Replaces I/l with 1 in numeric sections.
    - Formats into standard format (e.g., TN60 20000001759).
    """
    if not dl_str:
        return "Not Found"
...
def extract_issuing_authority(lines: list[str], full_text: str) -> str:
    """
    Extracts the government heading issuing authority.
    Example: UNION OF INDIA
    """
    for line in lines:
        if re.search(r'(?i)\b(union\s+of\s+india|government\s+of\s+[a-z\s]+|govt\s+of\s+[a-z\s]+)\b', line):
            return line.strip().upper()

    # Fallback to general RTO or Government pattern
...
def extract_document_type(lines: list[str], full_text: str) -> str:
    """
    Extracts document type containing 'Driving Licence'.
    Example: Driving Licence (Tamil Nadu)
    """
    for line in lines:
...
def extract_document_number(lines: list[str], full_text: str) -> str:
    """
    Locates DL No / DL. No label and searches nearby lines for document number.
    Handles TN6O -> TN60 OCR normalization.
    """
...
def extract_full_name(lines: list[str]) -> str:
    """
    Locates 'Name' label and searches the next 6 OCR lines.
    Ignores dates, blood groups, single letters, numbers, empty lines, and other labels.
    Returns the first line containing only uppercase alphabetic words and spaces.
    """
    blood_groups = r'^(0\+|o\+|0-|o-|a\+|a-|b\+|b-|ab\+|ab-)$'

    for i, line in enumerate(lines):
        if re.search(r'(?i)^\s*name\b', line) or re.search(r'(?i)\bname\b', line):
            # Check same line after Name label
            ...
            # Search next 6 OCR lines
            window = lines[i + 1:min(i + 7, len(lines))]
            for window_line in window:
                cand = window_line.strip()
                if not cand:
                    continue
                if re.search(r'\d', cand): # Ignore dates and numbers
                    continue
                if re.match(blood_groups, cand, re.IGNORECASE): # Ignore blood groups
                    continue
                if len(cand) < 2: # Ignore single letters
                    continue
                if re.search(KNOWN_LABELS_PATTERN, cand): # Ignore other labels
                    continue

                # Must contain only alphabetic uppercase words and spaces
...
def extract_date_of_birth(lines: list[str], full_text: str) -> str:
    """
    Locates 'Date of Birth' or 'DOB' label and searches nearby lines for valid DD-MM-YYYY date.
    """
...
def extract_issue_date(lines: list[str], full_text: str) -> str:
    """
    Locates 'Date of Issue' label and searches nearby lines for valid DD-MM-YYYY date.
    """
    date_regex = r'\b(\d{2}[/\.-]\d{2}[/\.-]\d{4}|\d{4}[/\.-]\d{2}[/\.-]\d{2})\b'
...
def extract_expiry_date(lines: list[str], full_text: str, issue_date: str, dob: str) -> str:
    """
    Locates 'Valid Till' or 'Expiry Date' label and searches nearby lines for valid DD-MM-YYYY date.
    Ensures issue_date and dob are not re-used.
    """
    date_regex = r'\b(\d{2}[/\.-]\d{2}[/\.-]\d{4}|\d{4}[/\.-]\d{2}[/\.-]\d{2})\b'
...
def parse_driving_license_text(ocr_text: str) -> dict:
    """
    Parses raw OCR text into required 7 identity fields.

    Args:
        ocr_text (str): Raw extracted OCR text.

    Returns:
        dict: Extracted structured fields matching required key names.
    """
    if not ocr_text:
        return {
    ...
    # Step 1: Pre-process & Normalize OCR text
    normalized_text = normalize_ocr_text(ocr_text)
    lines = [line.strip() for line in normalized_text.split('\n') if line.strip()]

    # Step 2: Invoke Helper Functions

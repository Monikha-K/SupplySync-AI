...
try {
  if (process.env.GEMINI_API_KEY) {
    ...
    console.log('✅ Gemini AI model loaded (gemini-1.5-flash)');
  } else {
    console.log('ℹ️  No GEMINI_API_KEY — using rule-based AI responses.');
  }
} catch (e) {
  console.error('⚠️  Gemini AI init failed:', e.message);
}
...
// ─── Sample Dataset (In-Memory Fallback) ─────────────────────────────────────
...
async function initMongoDB() {
  ...
  if (!uri || uri.includes('<db_password>')) {
    console.log("ℹ️  No valid MongoDB URI. Running with In-Memory Demo Store.");
    mongoErrorMsg = "Atlas password not configured in .env";
    return;
  }

  try {
    console.log("⚡ Connecting to MongoDB Atlas...");
    dbClient = new MongoClient(uri, { serverSelectionTimeoutMS: 6000 });
...
app.get('/api/shipment/:id', async (req, res) => {
  try {
    ...
    if (shipment) {
      ...
      res.status(404).json({
        success: false,
        message: `Shipment '${req.params.id}' not found. Try SHP1001, SHP1002, SHP1003, or SHP1004.`
      });
    }
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
...
app.post('/api/simulate-step', async (req, res) => {
  ...
  if (newProgress >= 100) {
    ...
    if (shipment.timeline) shipment.timeline.forEach(t => t.status = 'completed');
  ...
  shipment.eta = newEta;
  shipment.lastUpdated = new Date().toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })
    + ' • ' + new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
  shipment.summary = `Shipment ${shipment.shipmentId} is progressing towards ${shipment.destination}. Current position: ${newLocation}. ETA: ${newEta}.`;
  ...
  res.json({
    success: true,
    shipment,
    log: `[${new Date().toLocaleTimeString()}] Live Pulse: Progress ${newProgress}% • Location: ${newLocation} • Speed: ${shipment.telemetry.speed}`
  });
});
...
app.post('/api/seed', async (req, res) => {
  if (!isConnectedToAtlas || !db) {
    return res.status(400).json({
      success: false,
      message: "Cannot seed MongoDB Atlas: Connection not active."
    });
  }
  ...
  try {
    ...
    await collection.insertMany(SAMPLE_SHIPMENTS);
    res.json({ success: true, message: "Successfully seeded MongoDB Atlas!", count: SAMPLE_SHIPMENTS.length });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
});
...
app.post('/api/ai-query', async (req, res) => {
  ...
  if (!shipment) return res.status(404).json({ success: false, answer: 'Shipment context not found.' });

  // ── Try Gemini AI first ───────────────────────────────────────────────────
  if (geminiModel && query) {
    try {
      const prompt = `You are SupplySync AI, an expert logistics intelligence agent.

SHIPMENT CONTEXT:
- ID: ${shipment.shipmentId}
- Driver: ${shipment.driverName} | Phone: ${shipment.driverPhone} | Vehicle: ${shipment.vehicleNumber}
- Route: ${shipment.pickup} → ${shipment.destination}
- Current Location: ${shipment.currentLocation}
- Status: ${shipment.status} | Progress: ${shipment.progress}%
- ETA: ${shipment.eta} | Risk Level: ${shipment.riskLevel}
- Speed: ${shipment.telemetry.speed} | Cargo Temp: ${shipment.telemetry.temperature}
- Distance: ${shipment.telemetry.distanceCovered} covered of ${shipment.telemetry.totalDistance}
- Last Updated: ${shipment.lastUpdated}
- Summary: ${shipment.summary}

USER QUERY: "${query}"

Respond as a professional logistics AI agent. Be concise (2-3 sentences). Use 1-2 relevant emojis. Include specific data from the shipment context above.`;

      const result = await geminiModel.generateContent(prompt);
      const answer = result.response.text();
      return res.json({ success: true, answer, shipmentId: shipment.shipmentId, source: 'gemini' });
    } catch (err) {
      console.error('Gemini AI query error:', err.message);
  ...
  if (q.includes('delay') || q.includes('late') || q.includes('risk')) {
    if (shipment.status === 'Delayed') {
      answer = `⚠️ Warning: Shipment ${shipment.shipmentId} is delayed ~45 min near ${shipment.currentLocation} due to traffic. ETA: ${shipment.eta}.`;
    } else if (shipment.status === 'Delivered') {
      answer = `✅ No risks! Shipment ${shipment.shipmentId} has been safely delivered to ${shipment.destination}.`;
    } else {
      answer = `🟢 No delivery risks for ${shipment.shipmentId}. Cruising at ${shipment.telemetry.speed} near ${shipment.currentLocation}.`;
    }
    ...
    answer = `🌡️ Cargo temperature: ${shipment.telemetry.temperature}. Route near ${shipment.currentLocation} is clear.`;
  } else if (q.includes('speed') || q.includes('fast')) {
    answer = `🚚 Current speed: ${shipment.telemetry.speed}. Distance covered: ${shipment.telemetry.distanceCovered} of ${shipment.telemetry.totalDistance}.`;
  } else {
    answer = `🤖 AI Agent: ${shipment.shipmentId} is at ${shipment.currentLocation} → ${shipment.destination}. Progress: ${shipment.progress}%. ETA: ${shipment.eta}. ${shipment.summary}`;
  }

  res.json({ success: true, answer, shipmentId: shipment.shipmentId, source: 'rule-based' });
});
...
(async () => {
  ...
  const server = app.listen(PORT, () => {
    console.log(`\n🚀 SupplySync AI Backend running at http://localhost:${PORT}`);
    console.log(`📡 MongoDB: ${isConnectedToAtlas ? '✅ Atlas Connected' : '⚠️  Demo Mode (In-Memory)'}`);
    console.log(`🌐 Accepting requests from: ${FRONTEND_ORIGIN}\n`);
  });

  server.on('error', (err) => {
    if (err.code === 'EADDRINUSE') {
      console.log(`⚠️  Port ${PORT} already in use. Please kill the existing process.`);
    } else {
